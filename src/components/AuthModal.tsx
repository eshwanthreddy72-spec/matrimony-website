...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const prefill = (type: 'priya' | 'rohit' | 'admin') => {
    setError(null);
    if (type === 'priya') {
      setTab('login');
      setIdentifier('priya@example.com');
      setLoginPassword('Password123!');
    } else if (type === 'rohit') {
      setTab('login');
      setIdentifier('rohit@example.com');
      setLoginPassword('Password123!');
    } else if (type === 'admin') {
      setTab('admin');
      setIdentifier('admin@matrimony.com');
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`text-sm font-bold pb-2 border-b-2 transition-colors ${
                tab === 'login' ? 'border-rose-600 text-rose-600' : 'border-transparent text-slate-500 hover:text-slate-800'
              }`}
            >
              Member Login
            </button>
            <button
              ...
              className={`text-sm font-bold pb-2 border-b-2 transition-colors ${
                tab === 'register' ? 'border-rose-600 text-rose-600' : 'border-transparent text-slate-500 hover:text-slate-800'
              }`}
            >
              Register
            </button>
            <button
              ...
              className={`text-sm font-bold pb-2 border-b-2 transition-colors ${
                tab === 'admin' ? 'border-amber-600 text-amber-700' : 'border-transparent text-slate-500 hover:text-slate-800'
              }`}
            >
              Admin Portal
            </button>
        ...
        <div className="px-6 pt-4">
          ...
          <div className="mb-4 p-2.5 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1.5">
            <div className="flex items-center justify-between text-[11px] text-slate-500 font-semibold uppercase tracking-wider">
              ...
              <span className="text-rose-600">Click to fill</span>
            </div>
            <div className="flex flex-wrap gap-1.5">
              <button
                type="button"
                onClick={() => prefill('priya')}
                className="px-2 py-1 bg-white hover:bg-rose-50 hover:border-rose-200 border border-slate-200 rounded text-[11px] font-medium text-slate-700"
              >
                👤 Priya (User)
              </button>
              <button
                type="button"
                onClick={() => prefill('rohit')}
                className="px-2 py-1 bg-white hover:bg-rose-50 hover:border-rose-200 border border-slate-200 rounded text-[11px] font-medium text-slate-700"
              >
                👤 Rohit (User)
              </button>
              <button
                ...
                className="px-2 py-1 bg-amber-100 hover:bg-amber-200 border border-amber-300 rounded text-[11px] font-bold text-amber-900"
              >
                🛡️ Administrator
              </button>
            </div>
              ...
              {resetStep === 'request' ? (
                ...
                <form onSubmit={handleResetPassword} className="space-y-3">
                  <div className="p-2.5 bg-amber-50 rounded-xl border border-amber-200 text-xs text-amber-900">
                    ...
                    <p className="text-[11px] text-amber-700">Demo Code autofilled: {otp}</p>
                  </div>
